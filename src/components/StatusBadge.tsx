...
type BadgeType = 'role' | 'leave' | 'attendance_source' | 'status' | 'duty_status';
...
export default function StatusBadge({ type, value }: Props) {
  let color = 'bg-slate-800 text-slate-300 border border-slate-700';
  ...
  if (type === 'role') {
    ...
    switch (value) {
      ...
      case 'BERTUGAS':
        ...
        break;
      case 'MAINTENANCE':
        color = 'bg-amber-950/80 text-amber-300 border border-amber-800/50 font-semibold';
        break;
    }
    return (
      <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs ${color}`}>
        {DUTY_STATUS_LABELS[value] || value}
      </span>
    );
  }
